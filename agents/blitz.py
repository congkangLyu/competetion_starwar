"""
Orbit Wars - Parametric Heuristic Agent (blitz preset)

Adapted from the EDA notebook in `information/`. The agent scores every
(source_planet, target_planet) pair with a weighted sum of features and
attacks the highest-scoring target it can afford. This file ships with
the `blitz` preset, which won the 10-strategy round-robin tournament in
the EDA notebook with a 72.2% winrate.

Key improvements over the naive sniper:
  - Iterative capture-cost estimate (accounts for production during travel)
  - Sun-avoidance via segment-circle intersection
  - Optional defensive reinforcement (disabled in blitz)
  - Per-planet scoring with tunable weights

Score function (per source/target pair):

    score = -dist_weight  * dist/141
          + prod_weight   * prod/5
          - weak_weight   * ships/500
          + neutral_bonus * I[neutral]
          + enemy_bonus   * I[enemy]

Note: this file is structured to allow future parameter sweeps. The
BEGIN_STRATEGY / END_STRATEGY markers let an external script swap the
preset without touching the rest of the code.
"""
...
MAX_DIAG   = 141.421
MAX_SHIPS  = 500.0
WAIT_TURNS = 2  # let the home planet build up before launching


def fleet_speed(ships):
    """Speed = 1 + 5 * (log(ships)/log(1000))^1.5, capped at 6.0."""
    if ships <= 1:
        return 1.0
...
def _capture_cost(sx, sy, tgt):
    """Iteratively solve for the fleet size needed to capture `tgt` from
    (sx, sy), accounting for ships produced by `tgt` during transit.

    Fixed-point: needed = garrison + production * (dist / speed(needed)) + 1.
...
def _detect_threats(my_planets, fleets, player):
    ...
    A fleet is considered to be heading toward a planet if the planet lies
    in front of the fleet's heading and within (planet_radius + 3) of its
    line of travel.
    """
    threats = {}
    for f in fleets:
...
def make_fresh_agent():
    """Return a stateful agent function (resets the turn counter).

    Wrapped this way so the same module can be re-used across multiple
    games during local evaluation without state leaking between games.
    """
    s = STRATEGY
    tc = [0]
    ...
    def _agent(obs):
        ...
        # ── Attack pass ───────────────────────────────────────────────────
        # Sources are processed richest-first. Each source picks the
        # highest-scoring target it can afford (without re-targeting a
        # planet another source already chose this turn).
        targeted = set()
        for src in sorted(my_pl, key=lambda p: -avail[p.id]):
            ...
            if not scored:
                # No legal targets -- consolidate ships toward a friend
                if avail[src.id] > 15:
